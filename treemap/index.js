async function DrawMap() {
	...
	// compute the layout
	const root = d3.treemap().size([960, 570]).padding(1).round(true)(
		d3
	...
	const leaf = svg
		...
		.data(root.leaves())
		.join('g')
		.attr('transform', (d) => `translate(${d.x0},${d.y0 + 100})`)
		.attr('id', 'leaf');
	...
	svg
		...
		.html((d) =>
			d.data.name
				...
				.map(
					(str, i) =>
						`<tspan x=${d.x0 + 1} y=${d.y0 + 110 + 11 * i}>${str}</tspan>`
				)
				.join('')
		)
		.attr('font-size', '10px')
	...
	legendWrapper
		.append('g')
		.attr('transform', `translate(420 ,${height - 200})`)
		.attr('class', 'legend-item')
		.call(legend2);

	legendWrapper
		.append('g')
		.attr('transform', `translate(540 ,${height - 200})`)
		.attr('class', 'legend-item')
		.call(legend3);
